    ...
    <>
      ...
      >
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', height: 70, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div className="nav-links-desktop" style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
            {links.map(l => (
              <a key={l.label} href={l.href} className="nav-link" style={{ color: T.gray700, textDecoration: 'none', fontSize: 14, fontWeight: 500, transition: 'color 0.2s' }}>
                {l.label}
              </a>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <a href="https://wa.me/2341234567890" target="_blank" rel="noopener noreferrer"
              className="nav-whatsapp-desktop" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 14, fontWeight: 500, color: T.green600, textDecoration: 'none', transition: 'color 0.2s' }}
            ...
            <button onClick={() => setMenuOpen(!menuOpen)} className="nav-hamburger" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, display: 'none', flexDirection: 'column', gap: 5, alignItems: 'center' }}>
              {menuOpen ? (
                ...
                <FiMenu size={24} color={T.gray700} />
              )}
            </button>
          </div>
        </div>
      ...
      {menuOpen && (
        <div className="mobile-menu-overlay" onClick={() => setMenuOpen(false)} style={{
          position: 'fixed', top: 70, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.3)', zIndex: 799, animation: 'fadeIn 0.2s ease-in'
        }} />
      )}
      <div className={`mobile-menu-drawer ${menuOpen ? 'open' : ''}`} style={{
        position: 'fixed', top: 70, left: 0, right: 0, background: T.white, zIndex: 799, maxHeight: 'calc(100vh - 70px)', overflowY: 'auto',
        transform: menuOpen ? 'translateY(0)' : 'translateY(-100%)',
        transition: 'transform 0.3s cubic-bezier(.22,1,.36,1)',
        boxShadow: menuOpen ? `0 12px 32px rgba(13,43,31,0.12)` : 'none',
      }}>
        <div style={{ padding: '24px' }}>
          {links.map(l => (
            <a key={l.label} href={l.href} onClick={handleNavClick} style={{
              display: 'block', padding: '14px 16px', fontSize: 16, fontWeight: 500, color: T.green900, textDecoration: 'none',
              borderRadius: 8, transition: 'background 0.2s', marginBottom: 8,
            }} onMouseOver={e => e.currentTarget.style.background = T.green50} onMouseOut={e => e.currentTarget.style.background = 'transparent'}>
              {l.label}
            </a>
          ))}
          <a href="https://wa.me/2341234567890" target="_blank" rel="noopener noreferrer" onClick={handleNavClick} style={{
            display: 'flex', alignItems: 'center', gap: 8, padding: '14px 16px', fontSize: 15, fontWeight: 500, color: T.green600, textDecoration: 'none',
            borderRadius: 8, background: T.green50, marginTop: 12, marginBottom: 8,
          }}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/>
              <path d="M12 0C5.373 0 0 5.373 0 12c0 2.025.507 3.934 1.399 5.612L0 24l6.604-1.731A11.945 11.945 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.808 9.808 0 01-4.98-1.352l-.357-.212-3.922 1.029 1.045-3.819-.233-.371A9.818 9.818 0 012.182 12C2.182 6.58 6.58 2.182 12 2.182S21.818 6.58 21.818 12 17.42 21.818 12 21.818z"/>
          ...
          <button onClick={() => { onBook(); setMenuOpen(false); }} className="btn-primary" style={{ width: '100%', marginTop: 8, fontSize: 14 }}>
            Book Now
          </button>
        </div>
      </div>
